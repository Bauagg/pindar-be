...
export const deleteCardPublisher = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ code: 200, message: "Card publisher deleted successfully." });
  } catch (err) {
    next(err);
  }
};
...
export const deleteCardFeature = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ code: 200, message: "Card feature deleted successfully." });
  } catch (err) {
    next(err);
  }
};
...
export const getCreditCardList = async (req, res, next) => {
  try {
    ...
    const filters = {
      ...
      minYearlyIncome: minYearlyIncome
        ? parseFloat(minYearlyIncome)
        : undefined,
      maxYearlyIncome: maxYearlyIncome
        ? parseFloat(maxYearlyIncome)
        : undefined,
      sortBy,
      sortDirection,
...
export const deleteCreditCard = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ code: 200, message: "Credit card deleted successfully." });
  } catch (err) {
    next(err);
  }
};
  ...
  try {
    const filters = {
      ...
      featureIds: req.query.featureIds
        ? req.query.featureIds
            .split(",")
            .map((id) => id.trim())
            .filter((id) => /^[0-9a-fA-F\-]{36}$/.test(id))
        : [],
      minYearlyFee: req.query.minYearlyFee
        ? parseFloat(req.query.minYearlyFee)
        : undefined,
      maxYearlyFee: req.query.maxYearlyFee
        ? parseFloat(req.query.maxYearlyFee)
        : undefined,
      minYearlyIncome: req.query.minYearlyIncome
        ? parseFloat(req.query.minYearlyIncome)
        : undefined,
      maxYearlyIncome: req.query.maxYearlyIncome
        ? parseFloat(req.query.maxYearlyIncome)
        : undefined,
