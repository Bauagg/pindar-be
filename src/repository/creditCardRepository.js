  ...
  try {
    ...
    const { rows } = await client.query(
      ...
      [
        ...
        data.featureTypeId,
        data.yearlyFee,
        data.detailYearlyFee,
        data.title,
        data.additionalCardAnnualFee,
        data.purchaseRate,
        data.cashbackRate,
        data.detailCashbackRate,
        data.minimumWithdraw,
        data.monthlyIncomeMinimum,
        data.whoCanRegister,
        data.mustHaveCreditCard,
        data.createdBy,
        data.yearlyIncomeMinimum,
...
export const updateCreditCardById = async (id, data) => {
  ...
  try {
    ...
    const existing = await client.query(
      "SELECT image_id FROM credit_card WHERE id = $1",
      [id]
    );

    const existingImageId = existing.rows[0]?.image_id;
    const imageIdToUse = data.imageId ?? existingImageId;
    ...
    // ✅ Remove old features
    await client.query(
      `DELETE FROM credit_card_features WHERE credit_card_id = $1`,
      [id]
    );
...
export const deleteCreditCardById = async (id) => {
  await pool.query(`UPDATE credit_card SET is_deleted = TRUE WHERE id = $1`, [
    id,
  ]);
};
...
export const searchCreditCards = async (filters) => {
  ...
  if (publisherIds && publisherIds.length > 0) {
    const placeholders = publisherIds
