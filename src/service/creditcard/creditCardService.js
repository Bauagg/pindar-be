...
export const addCreditCard = async (data) => {
  ...
  if (!publisherExists) {
    throw {
      status: 400,
      message: "Invalid publisherId. Publisher does not exist.",
    };
  }
  ...
  if (!featureExists) {
    throw {
      status: 400,
      message: "Invalid featureTypeId. Feature does not exist.",
    };
  }
...
export const fetchCreditCardsList = async (filters) => {
  ...
  if (
    !["yearly_fee", "yearly_income_minimum", "created_date"].includes(sortBy)
  ) {
...
export const recordProductAccess = async (
  productType,
  productId,
  userId,
  req
...
const formatCreditCardResponse = (creditCard) => ({
  ...
  applyTutorial: creditCard.apply_tutorial,
  termAndConditions: creditCard.term_and_conditions,
