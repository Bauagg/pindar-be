...
export const getContentListPinned = async (req, res, next) => {
  try {
    const contentList = await fetchPinnedContent();
    res.status(200).json({
      code: 200,
      message: "Content list retrieved successfully.",
      data: contentList,
...
export const updatePinValue = async (req, res, next) => {
  try {
    const { id } = req.params;
    const updatedContent = await modifyContentUpdatePin(id, req.body);
    res.status(200).json({
      code: 200,
      message: "Content updated successfully.",
      data: updatedContent,
...
export const deleteContent = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ code: 200, message: "Content deleted successfully." });
  } catch (err) {
    next(err);
  }
};
  ...
  try {
    ...
    if (productType && !["lender", "credit_card"].includes(productType)) {
      return res.status(400).json({
        code: 400,
        message:
          'Invalid product type. Must be either "lender" or "credit_card".',
      });
    }
