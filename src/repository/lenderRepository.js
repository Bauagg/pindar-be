...
export const updateFileUsage = async (client, imageId) => {
  await client.query("UPDATE files SET is_used = true WHERE id = $1", [
    imageId,
  ]);
};

export const deleteOtherLenders = async (client, lenderId) => {
  await client.query("DELETE FROM other_lender WHERE lender_id = $1", [
    lenderId,
  ]);
};

export const checkLenderExists = async (client, lenderId) => {
  const result = await client.query("SELECT id FROM lender WHERE id = $1", [
    lenderId,
  ]);
...
export const validateParamExist = async (client, paramKey, paramGroup) => {
  ...
  if (rows.length === 0)
    throw { status: 400, message: `${paramKey} not found in ${paramGroup}` };
};

export const insertLender = async (
  client,
  {
    lenderName,
    directLink,
...
export const insertLenderDetail = async (
  client,
  {
    lenderId,
...
export const insertOtherLender = async (
  client,
  lenderId,
  relatedIds,
  relationType
...
export const updateLenderData = async (
  client,
  {
    id,
    lenderName,
  ...
  await client.query(
    `UPDATE lender 
     SET lender_name = $1, direct_link = $2, max_loan = $3, loan_type = $4, payment_type = $5, 
         max_tenor = $6, image_id = COALESCE($7, image_id), updated_by = $8, updated_date = NOW()
     WHERE id = $9`,
    [
      lenderName,
      directLink,
...
export const updateLenderDetail = async (
  client,
  { id, additionalInformation, termsDocument }
) => {
  await client.query(
